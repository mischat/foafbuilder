...
class GeoLatField extends Field{
	
	/*predicateUri is only appropriate for simple ones (one triple only)*/
	public function GeoLatField(){
		$this->label ='Location';
		$this->name = 'geoLat';
		$this->queryBit = '?x foaf:based_near ?l . ?l geo:lat ?geoLat';
		$this->type = 'literal';
	}
	...
	public function saveToModel(&$foafData, $value){
		...
		/*TODO: Probably inefficient, possibly a load method is needed to store the event uri in this object?*/
			$query = " 	PREFIX foaf: <http://xmlns.com/foaf/0.1/>
					...
					SELECT ?l ?x ?geoLat WHERE {?z foaf:primaryTopic ?x . ".$this->getQueryBit().".}";
		...
		if($results){
			...
			$remove_statement = new Statement(new BlankNode($results[0]['?l']->uri), new Resource("http://www.w3.org/2003/01/geo/wgs84_pos#lat"), new Literal($results[0]['?geoLat']->label));
			$foafData->getModel()->remove($remove_statement);
			
			/*and add another one if the value is set.*/
			if($value){
				$add_statement = new Statement(new BlankNode($results[0]['?l']->uri), new Resource("http://www.w3.org/2003/01/geo/wgs84_pos#lat"), new Literal($value));
			...
			if($value){
				...
				if($found_model->triples[0]){
					/*there is at least one based near triple, so loop through them adding the required lat triple*/
					foreach($found_model->triples as $triple){
						$add_statement = new Statement($triple->obj, new Resource("http://www.w3.org/2003/01/geo/wgs84_pos#lat"),new Literal($value));
						$foafData->getModel()->add($add_statement);
					}
				} else {
					/*there is no based near triple, so add it and add the required lat triple*/
					...
					$foafData->getModel()->add($add_statement);
					$add_statement = new Statement($new_bnode, new Resource("http://www.w3.org/2003/01/geo/wgs84_pos#lat"),new Literal($value));
				}
			}
		}
	}
